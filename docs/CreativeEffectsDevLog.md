...
5. **可選泡泡顏色**：`WaterParams` 加 `BubbleR/G/B`，renderer 的 `BlendWhite`→`BlendColor(...,cr,cg,cb)` 把亮邊 rim＋specular highlight（原寫死白）改用該色（lens 折射內容不變）。預設白＝行為不變，**由兩個 builder 設定**（`WaterSettings.ToParams`／`MorphSettings.ToWaterParams`，皆預設 255）——struct 預設 0,0,0 是黑，務必在 builder 設值。**Gotcha**：buffer 是 `"RGBA"`（idx=R, idx+1=G, idx+2=B），`BlendColor` 要對應，別寫成 BGR。兩 dialog 各加 `ColorDialog`＋`Panel` 色票（Panel 不在 theme 切換內 → 不被深色主題蓋掉），比照 jigsaw 色票。resx 鍵 `WaterDialog_BubbleColor`（三語＋Designer）。測試：`WaterFieldTests` 加 `MorphSettings.ToWaterParams` 顏色傳遞＋預設白兩例。
6. **時間窗陷阱修正（實機回報：起始設 15.5s 完全沒效果）**：(a) **water**——`Full ≤ Start`（如 Start 調到 15.5 但 Full 留預設 4.0）時，`EffectAlpha` 以過期的 Full 起算淡出 → 水還沒開始就已淡完、全程隱形。修正：`WaterField.EffectiveFull(start,full)=max(start,full)`，兩個 builder（play-along/frozen）改用之；frozen 的 fill phase 同步延伸 `duration=max(duration, effFull)`；`WaterDialog` OK 時驗證 Full > Start（resx 鍵 `WaterDialog_FullAfterStart` 三語＋Designer）。(b) **ripple**——play-along 的混合閘門 `t < DurationSeconds`（預設 4s）會把落在窗外的水滴（如 15.5s）整顆吞掉。修正：`RippleField.EffectiveDuration(duration,drops,m)=max(duration, TotalSeconds(...))` 自動延伸到「最晚水滴＋壽命」；play-along/frozen/`RunRipple` 的 outFrames 估算三處一致使用（TimeDamping=0 的無限壽命不延伸、仍由 duration 封頂）。**audit**：wind/rain/slot/quicksand 用共用 `GifEffectWindow.Clamp`（start 會滑回 clip 內）無此問題；morph 用相對時長（pre-roll/morph）無絕對起始陷阱；grid mosaic 無時間窗。測試：`RippleFieldTests.EffectiveDuration_ExtendsToCoverLateDrops`、`WaterFieldTests.EffectiveFull_ClampsFullToStart`。
