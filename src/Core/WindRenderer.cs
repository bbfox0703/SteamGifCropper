...
{
    // Applies a WindField displacement field to an image, one frame at a time. Kept as an independent
    // copy of the ripple resampler (so the ripple effect stays untouched; the two can be unified later
    // if we want). Per output pixel we inverse-map to a displaced source position and bilinearly sample
    // it, giving the refraction/sway look of wind rolling over a field. Done with direct RGBA byte
    // buffers + Parallel.For (no ImageMagick Drawables / Q8 Displace map), edge-clamped at the borders.
    public static class WindRenderer
    {
        // Renders `source` displaced by the wind field at absolute time t (seconds). Returns a new image
        // the same size as the source; the caller owns it.
        public static MagickImage RenderFrame(IMagickImage<byte> source, double t, WindGust[] gusts, WindMedium medium)
        {
            uint w = source.Width;
            ...
            Parallel.For(0, ih, y =>
            {
                ...
                for (int x = 0; x < iw; x++)
                {
                    var (dx, dy) = WindField.Displacement(x, y, t, gusts, medium);
